...
class TestInstall < Minitest::Test
  ...
  def test_full_returns_if_not_linux
    capture_io do
      ...
      non_linux { assert_equal(VpsCli::Install.full, :not_installed) }
    end
  end
  ...
  def test_full_raises_error_when_prep_errors
    ...
    capture_io do
      VpsCli::Install.stub(:all_install, raises_exception) do
        assert_raises(RuntimeError) { VpsCli::Install.full }
      end
    end
  end

  def test_returns_installed_with_no_errors
    VpsCli::Install.stub(:all_install, true) do
      linux { assert_equal(VpsCli::Install.full, :installed) }
    end
  end
end
